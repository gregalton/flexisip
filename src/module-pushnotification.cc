...
ModuleInfo<PushNotification> PushNotification::sInfo{
    ...
    [](GenericStruct& moduleConfig) {
	    ...
	    moduleConfig.get<ConfigBoolean>("google")->setDeprecated(
	        {"2020-01-28", "2.0.0",
	         "'google' push notification backend has been removed. Please use 'firebase' instead."});
	    moduleConfig.get<ConfigStringList>("google-projects-api-keys")
	        ->setDeprecated({"2020-01-28", "2.0.0", "This setting has no effect anymore."});
	    moduleConfig.get<ConfigDuration<chrono::seconds>>("time-to-live")
	    ...
	    moduleConfig.get<ConfigString>("add-to-tag-filter")
	        ->setDeprecated(
	            {"2021-12-30", "2.2.0",
	    ...
	    moduleConfig.get<ConfigBoolean>("windowsphone")
	        ->setDeprecated({"2023-07-15", "2.3.0", "Windows push are not handled anymore. This config does nothing."});
	    moduleConfig.get<ConfigString>("windowsphone-package-sid")
	        ->setDeprecated({"2023-07-15", "2.3.0", "Windows push are not handled anymore. This config does nothing."});
	    moduleConfig.get<ConfigString>("windowsphone-application-secret")
	        ->setDeprecated({"2023-07-15", "2.3.0", "Windows push are not handled anymore. This config does nothing."});
